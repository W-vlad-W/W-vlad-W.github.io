...
jQuery(document).ready(function(){
    ...
    jQuery(document).on('click','.ar_accordian', function(event){
...
ARModelFields.prototype.uploadButtonScript = function() {
    ...
    jQuery(function($) {    
        ...
        $(document).on('click','.upload_usdz_button, .upload_glb_button, .upload_skybox_button, .upload_environment_button, .upload_poster_button, .upload_qr_image_button, .upload_asset_texture_button, .upload_asset_texture_button_0, .upload_asset_texture_button_1, .upload_asset_texture_button_2, .upload_asset_texture_button_3, .upload_asset_texture_button_4, .upload_asset_texture_button_5, .upload_asset_texture_button_6, .upload_asset_texture_button_7, .upload_asset_texture_button_8, .upload_asset_texture_button_9', function(e) {
            ...
            $('#uploader_modelid').val(model_idd.replace(/"/g, ''));
                ...
                var attachments = custom_uploader.state().get('selection').map( 
                 ...
                 $.each(attachments, function( index, attachement ) {
                    ...
                    if ((filetype === 'usdz') || (filetype === 'USDZ') || (filetype === 'lity') || (filetype === 'LITY')){
                        ...
                        else if (window.button_clicked.indexOf('upload_poster_button') != -1){
                            $('#_ar_poster' + sffx).val(fileurl).trigger('change');
                        }
...
ARModelFields.prototype.assetBuilder = function(){
    ...
    var updatear_asset_size_options = function(ratio) {
        ...
        if (ratio in modelTypeJsonList) {
            var ar_asset_default_size = parseFloat(document.querySelector("#_ar_asset_default_size").value).toFixed(2); // Round to 2 decimals

            for (var i = 0; i < modelTypeJsonList[ratio].length; i++) {
                var modelTypeID = parseFloat(modelTypeJsonList[ratio][i].modelTypeID).toFixed(2); // Round to 2 decimals
                var selected = (ar_asset_default_size === modelTypeID) ? "selected" : ""; // Check if they match
                listItems += "<option value='" + modelTypeJsonList[ratio][i].modelTypeID + "' " + selected + ">" + modelTypeJsonList[ratio][i].modelType + "</option>";
                // If selected, also set the value of #_ar_x to modelTypeID
                if (selected) {
                    document.getElementById("_ar_x").value = modelTypeJsonList[ratio][i].modelTypeID;
                }
            }
            ...
            if (document.getElementById("_asset_texture_file_0").value) {
                ...
                const modelViewer = document.querySelector('#model_'+self.modelid);
    ...
    jQuery(document).ready(function($) {
        // Initialize the Iris color picker with custom palettes for #_ar_light_color
        $('#_ar_light_color').iris({
            palettes: ['#ff0000', '#00ff00', '#0000ff', '#ffffff', '#000000', '#cccccc'],
            change: function(event, ui) {
                // Detect color change and update the model viewer
                var modelViewer = document.querySelector('#model_' + self.modelid);
                modelViewer.setAttribute("light-color", ui.color.toString());
            },
            clear: function() {
                // Handle color clear event
                var modelViewer = document.querySelector('#model_' + self.modelid);
                modelViewer.setAttribute("light-color", "");
            }
        });
    
        // Toggle Iris color picker on #_ar_light_color input field click
        $('#_ar_light_color').on('click', function(event) {
            event.stopPropagation(); // Prevent this click from triggering the document click event
            if ($('.iris-picker').is(':visible')) {
                $('#_ar_light_color').iris('hide');
            } else {
                $('#_ar_light_color').iris('show');
            }
        });
    
        // Close the color picker when clicking outside of it
        $(document).on('click', function(event) {
            if (!$(event.target).closest('#_ar_light_color, .iris-picker').length) {
                $('#_ar_light_color').iris('hide'); // Hide the Iris color picker if clicking outside
            }
        });
    });
    ...
    // Ensure the script runs after the DOM is fully loaded
        $(document).ready(function() {
            ...
            if($('#_ar_frame_color').val()){
                currentColor = $('#_ar_frame_color').val().trim(); // Default to black if empty
            } else {        
            ...
            // Toggle Iris color picker on input field click
            $('#_ar_frame_color').on('click', function(event) {
                event.stopPropagation(); // Prevent this click from triggering the document click event
                if ($('.iris-picker').is(':visible')) {
                    $('#_ar_frame_color').iris('hide');
                } else {
                    $('#_ar_frame_color').iris('show');
                }
            });
        
            // Close the color picker when clicking outside of it
            $(document).on('click', function(event) {
                if (!$(event.target).closest('#_ar_frame_color, .iris-picker').length) {
                    $('#_ar_frame_color').iris('hide'); // Hide the Iris color picker if clicking outside
                }
            });
        });
        ...
        $('#_ar_framed, #_ar_frame_opacity, #_asset_texture_file_0').on('change', function() {
            updateModelViewer();
        });
    ...
    jQuery("select#ar_asset_size").on('change',function(){
        ...
        if (selectedSize != null){
            //console.log(selectedSize + ' selected asset size');
        ...
        const modelViewer = document.querySelector('#model_'+self.modelid);
        var x = document.getElementById('_ar_x') ? document.getElementById('_ar_x').value : '1';
        var y = document.getElementById('_ar_y') ? document.getElementById('_ar_y').value : '1';
        var z = document.getElementById('_ar_z') ? document.getElementById('_ar_z').value : '1';
    ...
    // Decode the URL-encoded blob URL
        var decodedImageUrl = decodeURIComponent(imageUrl); // Decode the URL-encoded blob URL
        
        // Create an image element dynamically
        var img = new Image();
        
        // Check if the decoded URL is a blob URL
        if (decodedImageUrl && decodedImageUrl.startsWith('blob:')) {
            // If it's a blob URL, use it directly
            img.src = decodedImageUrl;  // Blob URL
        } else {
            img.src = imageUrl;
        }
    ...
    // Trigger the function when the value of _asset_texture_file_0 changes (on file selection or clearing)
    jQuery('#_asset_texture_file_0').on('change', function() {
        calculateImageRatio();
        asset_display_thumb();
    });
    
    // Also trigger the function on text input (useful if this applies to non-file fields)
    jQuery('#_asset_texture_file_0').on('input', function() {
        calculateImageRatio();
        asset_display_thumb();
    }); 
};
...
ARModelFields.prototype.modelFields = function(){
    ...
    document.getElementById('_ar_hide_dimensions').addEventListener('change', function() {
        ...
        jQuery('.ar-dimension-control').toggleClass('nodisplay');
...
jQuery(document).ready(function($) {
    ...
    // Ensure the script runs after the DOM is fully loaded
        $(document).ready(function() {
            ...
            if($('#_ar_frame_color').val()){
                currentColor = $('#_ar_frame_color').val().trim(); // Default to black if empty
            }else if($('#ar_progress_bar_color').val()){
                currentColor = $('#ar_progress_bar_color').val().trim(); // Default to black if empty
            } else {        
            // Check if the color value is correctly set            
                currentColor = '#666666'; // Fallback to black if still empty, null, or falsy
            }

            // Initialize the color picker with the current color
            $('#ar_progress_bar_color').iris({
                color: currentColor,
                change: function(event, ui) {
                    // Handle color change event for frame color
                    //updateModelViewer();
            ...
            // Toggle Iris color picker on input field click
            $('#ar_progress_bar_color').on('click', function(event) {
                
                event.stopPropagation(); // Prevent this click from triggering the document click event
                if ($('.iris-picker').is(':visible')) {
                    $('#ar_progress_bar_color').iris('hide');
                } else {
                    $('#ar_progress_bar_color').iris('show');
                }
            });
        
            // Close the color picker when clicking outside of it
            $(document).on('click', function(event) {
                if (!$(event.target).closest('#ar_progress_bar_color, .iris-picker').length) {
                    $('#ar_progress_bar_color').iris('hide'); // Hide the Iris color picker if clicking outside
                }
            });
