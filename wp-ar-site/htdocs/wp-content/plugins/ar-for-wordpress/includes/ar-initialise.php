...
if (!function_exists('ar_advance_register_script')){
    function ar_advance_register_script() {
        global $ar_plugin_root_url, $ar_version;
        // Enqueue necessary styles and scripts

        
        
        wp_enqueue_script('iris', admin_url('js/iris.min.js'), array('jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch'), '1.0.0', true);
        wp_enqueue_script('ar_admin', $ar_plugin_root_url.'assets/js/ar-admin.js', array('jquery','iris'), $ar_version, false);

        wp_script_add_data('ar_admin', 'async', true);
        
        wp_enqueue_script('ar_model', $ar_plugin_root_url.'assets/js/ar-model.js', array('jquery'), $ar_version, false);
        wp_script_add_data('ar_model', 'async', true);

        wp_enqueue_script('ar_copy_to_clipboard', $ar_plugin_root_url.'assets/js/ar-display.js', array('jquery'), $ar_version, true);
        wp_script_add_data('ar_copy_to_clipboard', 'async', true);
    }
}
...
if (!function_exists('ar_advance_register_style')){
    function ar_advance_register_style() {
        global $ar_plugin_root_url, $ar_version;
        wp_enqueue_style('ar_styles', $ar_plugin_root_url.'assets/css/ar-display.css', false, $ar_version, 'all'); 
        //Custom CSS now called in ar-class.php when model is loaded depending if custom css used in settings or for that model
        //wp_enqueue_style('ar_styles_custom', plugins_url('ar-for-wordpress/assets/css/ar-display-custom.css'), false, '1.0.0', 'all'); 
    }
}
...
if (!function_exists('load_model_viewer_js')){
    function load_model_viewer_js(){
        global $ar_plugin_root_url;
        wp_enqueue_script( 'ar-model-viewer-js', $ar_plugin_root_url.'assets/js/model-viewer.min.js', array(), '1.0.0', array('in-footer'=>true,'strategy' => 'async') );
    }
}
...
if (!function_exists('load_model_viewer_components_js')){
    function load_model_viewer_components_js(){
        global $ar_plugin_root_url;
        wp_enqueue_script( 'ar-model-viewer-webcom-js', $ar_plugin_root_url.'assets/js/webcomponents-loader.js', array(), '1.0.0', array('in-footer'=>true,'strategy' => 'async')  );
        wp_enqueue_script( 'ar-model-viewer-intersection-js', $ar_plugin_root_url.'assets/js/intersection-observer.js', array(), '1.0.0', array('in-footer'=>true,'strategy' => 'async')  );
        wp_enqueue_script( 'ar-model-viewer-resize-js', $ar_plugin_root_url.'assets/js/ResizeObserver.js', array(), '1.0.0', array('in-footer'=>true,'strategy' => 'async')  );
    }
}
...
if (!function_exists('ar_allowed_html')){
    function ar_allowed_html(){
        ...
        $allowed_html = array(
            ...
            // Allow <style> tag
            'style' => array(
                'type' => true, // Allow the 'type' attribute for style
            ),
            // Allow <script> tag with attributes
            'script' => array(
                'type' => true, // Allow the 'type' attribute for script
                'src' => true,  // Allow the 'src' attribute for script
            ),
        );
...
//exclude ar-gallery url from hide my wp ghost lite plugin

if(!function_exists('disable_hmwp_on_ar_gallery_urls')){
    function disable_hmwp_on_ar_gallery_urls($process) {
        // Check if the current request URL contains a specific plugin path
        if (strpos($_SERVER['REQUEST_URI'], 'wp-content/plugins/ar-for-wordpress/includes/ar-gallery.php') !== false || strpos($_SERVER['REQUEST_URI'], 'wp-content/plugins/ar-for-woocommerce/includes/ar-gallery.php') !== false) {            
            return false;
        }
    ...
    add_filter('hmwp_process_init', 'disable_hmwp_on_ar_gallery_urls');
}

if(!function_exists('whitelist_file_in_wp_content')){

    function whitelist_file_in_wp_content( $whitelist ) {
        $whitelist[] = 'ar-gallery.php';
        return $whitelist;
    }
