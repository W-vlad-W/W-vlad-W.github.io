...
if (!function_exists('ar_subscription_setting')){
    function ar_subscription_setting() {
        global $wpdb, $ar_version, $ar_plugin_id, $ar_wc_active, $ar_wp_active, $ar_rate_this_plugin, $shortcode_examples, $shortcode_examples_wc, $woocommerce_featured_image, $ar_whitelabel, $ar_css_styles, $ar_css_names, $ar_plugin_root_url;
        ...
        if ($_POST){
            ...
            $settings_fields = array(
                'ar_licence_key', 'ar_wl_file', 'ar_view_file', 'ar_qr_file', 'ar_qr_destination', 'ar_view_in_ar', 
                'ar_view_in_3d', 'ar_dimensions_units', 'ar_dimensions_label', 'ar_user_upload_button', 'ar_user_default', 
                'ar_user_default_image', 'ar_user_button', 'ar_user_modelviewer', 'ar_fullscreen_file', 'ar_play_file', 
                'ar_pause_file', 'ar_dimensions_inches', 'ar_hide_dimensions', 'ar_no_posts', 'ar_hide_arview', 
                'ar_hide_qrcode', 'ar_hide_reset', 'ar_hide_fullscreen', 'ar_hide_gallery_sizes', 'ar_scene_viewer', 
                'ar_css', 'ar_css_positions', 'ar_open_tabs_remember', 'ar_secure_model_urls', 'ar_progress_bar_color'
            );
            
            foreach ($settings_fields as $v) {
                if (!isset($_POST[$v])) {
                    $_POST[$v] = '';
                }
            
                if ($v == 'ar_css_positions' && is_array($_POST[$v])) {
                    // Sanitize each element in the array individually
                    $sanitized_array = array_map('sanitize_text_field', wp_unslash($_POST[$v]));
                    update_option($v, $sanitized_array);
                } else {
                    // For non-array fields, sanitize as a single text field
                    update_option($v, sanitize_text_field(wp_unslash($_POST[$v])));
                }
            }
            //Delete the custom css file which will be automatically recreated next time a model is viewed
            if (file_exists(wp_upload_dir()['basedir'].'/ar-custom-styles.css')){
                wp_delete_file(wp_upload_dir()['basedir'].'/ar-custom-styles.css');
            }
        }
        //Delete Post
        ...
        <?php }?>
            <div id="licence_page" class="licence_page">
                  ...
                  <div>
                      ...
                      if ($disabled!=''){
                          if ($ar_licence_key==''){
                              ...
                              $alert = wp_kses('Invalid or Expired Licence Key - '.ar_licence_check(), ar_allowed_html());
                          }
                      }
                ...
                <?php if ($alert!=''){
                    echo '<br clear="all"><br><p><b>'.esc_html($alert). '</b></p><br>';
                    if ($disabled!=''){
                        if ($ar_licence_key==''){
                ...
                <div id="ar_options_panel" class="ar_accordian_panel">
                    ...
                    <div>
                          <div style="min-width:160px;float:left;"><strong>
                              <?php
                                echo wp_kses('Custom Play Button', ar_allowed_html());
                ...
                <div id="ar_element_positions_panel" class="ar_accordian_panel">
                    ...
                    <div>
                        <div style="width:50%;float:left;"><strong>
                        <?php
                        echo wp_kses('Progress Bar Color', ar_allowed_html());
                        
                        ?> </strong><br><input id="ar_progress_bar_color" name="ar_progress_bar_color" type="text" <?php echo  esc_html($disabled);?> value="<?php echo esc_html(get_option('ar_progress_bar_color')); ?>"></div>
                    </div>
                    <br  clear="all"><br>
                    <div>
                        <div style="width:50%;float:left;"><strong>
                ...
                if ($ar_wc_active==true){ 
                    ...
                    <div id="ar_woocommerce_featured_panel" class="ar_accordian_panel">
                        <br>
                            <h3><?php echo wp_kses('Woocommerce Featured Product Image Templates', ar_allowed_html());?></h3>
                            <?php echo wp_kses('Copy one of the following single product woocommerce templates found in the AR for Woocommerce plugin "templates" folder to your theme. The Product Model template will display the AR model as the featured image, whilst the Gallery Builder template will display your featured image as a 3D model artwork to hang on the wall.', ar_allowed_html());?></p>
                            <?php 
                            ...
                            if (isset($_POST['delete_template_file'])){
                                ...
                                echo '<b>Template file already in place.</b>';
                                   // check_and_delete_woocommerce_template();
        ...
        */
        ...
        if (substr($licence_result,0,5)!='Valid'){?>
            <div style="float:left;width:45%"><a href="https://augmentedrealityplugins.com" target="_blank"><img src="<?php echo esc_url( plugins_url( '../assets/images/ar_wordpress_ad.jpg', __FILE__ ) ); ?>" style="padding:10px 10px 10px 0px;"><img src="<?php echo esc_url( plugins_url( '../assets/images/ar_woocommerce_ad.jpg', __FILE__ ) ); ?>" style="padding:10px 10px 10px 0px;"></a></div>
        <?php } 
        }
        wp_enqueue_media();
        ?>
...
if (!function_exists('ar_licence_check')){
    function ar_licence_check() {
        global $wpdb;
        $link = 'https://augmentedrealityplugins.com/ar/ar_subscription_licence_check_upd.php';
        ob_start();
        $model_count = ar_model_count();
        ...
        if ($licence_key!=''){
            ...
            if ((is_wp_error($response)) AND (isset($response['body'])) AND ($response['body'] !== '') AND (!ctype_space($response['body']))) {
                return $response['body'];
            }else{
                $curl_check = trim(ar_curl($link.'?licence_key='.get_option('ar_licence_key').'&model_count='.$model_count.'&domain_name='.site_url()));
                if ($curl_check){
                    return $curl_check;
                }else{
                    return 'error';
...
if (!function_exists('ar_upgrade_banner')){
    function ar_upgrade_banner() { 
        global $ar_plugin_id, $ar_plugin_root_url; 
        ...
        <div id="ar_shortcode_instructions" class="notice notice-warning is-dismissible">
                    <div style="width:100%;background-color:#12383d">
                      ...
                      <?php  
                        ...
                        echo '<img src="'.esc_url( $ar_plugin_root_url.( "/assets/images/".$ar_plugin_id."-box.jpg" ) ).'" style="padding: 10px 30px 10px 10px; height:60px" align="left">
...
if (!function_exists('ar_show_update_notice')){
    function ar_show_update_notice() {
        ...
        if (get_transient('ar_show_update_notice')) {
            ...
            require_once(plugin_dir_path(__FILE__). 'ar-banner.php');
            ?>
