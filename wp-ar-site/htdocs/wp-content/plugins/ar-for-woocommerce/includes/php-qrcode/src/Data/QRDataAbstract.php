...
abstract class QRDataAbstract implements QRDataInterface{
	...
	protected function getLengthBits():int{
		...
		throw new QRCodeDataException(sprintf('invalid version number: %d', esc_html($this->version)));
	}
	...
	protected function getMinimumVersion():int{
		...
		throw new QRCodeDataException(sprintf('data exceeds %d characters', esc_html($maxlength)));
	}
	...
	protected function writeBitBuffer(string $data):void{
		...
		if($this->bitBuffer->getLength() > $MAX_BITS){
			throw new QRCodeDataException(sprintf('code length overflow. (%d > %d bit)', esc_html($this->bitBuffer->getLength()), esc_html($MAX_BITS)));
		}
