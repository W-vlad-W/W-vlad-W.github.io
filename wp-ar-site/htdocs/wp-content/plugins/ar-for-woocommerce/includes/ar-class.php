...
class AR_Plugin {
    ...
    public function ar_display_shortcode($atts, $variation_id='') {
        ...
        if ((get_option('ar_licence_valid')=='Valid')OR($model_count<=1)){
            ...
            if (!$glb_file && !is_admin()) { // If glb file is not found for the main product
    ...
    public function ar_display_popup($model_array){
        ...
        $ar_model_id = $model_array['model_id'];
        if (strpos($ar_model_id, ',') !== false) {
            $ar_model_id_array = explode(',', $ar_model_id);
            $ar_model_id = str_replace(" ","",implode("_",$ar_model_id_array));
        }
        ...
        if ((!isset($model_array['ar_hide_fullscreen']))OR($model_array['ar_hide_fullscreen']=='')){
            ...
            $popup_output ='
            <div id="ar_popup_'.$ar_model_id.'" class="ar_popup">
                <div class="ar_popup-content">
                    '.$this->ar_display_model_viewer($model_array, $model_array['model_id']).'
                </div>
            </div>';

            add_action( 'wp_footer', function( $arg ) use ( $popup_output,$model_array,$ar_model_id ) {
                ...
                <script>  
                    jQuery(document).ready(function(){       
                        var options = {                                                   
                            ...
                            id: '<?php echo esc_html($ar_model_id);?>',
                        };
                        
                        var model_<?php echo esc_html($ar_model_id);?>_pop = ARModelViewer('<?php echo esc_html($ar_model_id);?>_pop',options);
                    });

                </script>
                <?php
            } );
            add_action( 'admin_footer', function( $arg ) use ( $popup_output,$model_array,$ar_model_id ) {
                echo wp_kses($popup_output, ar_allowed_html());
                ?>
                <script> 
                    jQuery(document).ready(function(){         
    ...
    public function ar_user_upload($atts) { 
        if (get_option('ar_licence_valid') == 'Valid') {
            ...
            $model_array['ar_hide_dimensions'] = '';
            $model_array['model_id'] = 'user_upload';
            $atts['hotspots_html'] = $this->get_ar_hotspots($model_array);
            //$atts['qrcode_html'] = $this->get_ar_qr_html($model_array);
    ...
    public function ar_display_model_viewer($model_array, $atts_id=''){
        global $wp, $ar_plugin_id, $ar_whitelabel, $ar_css_names, $ar_css_styles, $model_viewer_js_loaded, $ar_scale_js;
        if (!is_admin()) {
        ...
        if (get_post_meta( $model_id, '_ar_poster', true )){
            $model_array['ar_poster']=get_post_meta( $model_id, '_ar_poster', true ).'"';
        }else{
            $model_array['ar_poster']='';
        }
        ...
        //override scene-viewer if animation is set
        if ($model_array['ar_animation_selection'] !=''){
            $viewers = 'webxr scene-viewer quick-look';
        }
        ...
        $model_array['pause_btn'] = $pause_btn;
        ...
        //}
        $this->get_ar_custom_css($model_array);
        
        ob_start();
        $this->load_template( 'model-viewer', $args  );
        return $model_viewer_js.ob_get_clean();
    ...
    public function get_ar_qr_html($model_array){
        ...
        //check if qr destination is override in the model 
        $ar_qr_destination = $model_array['ar_qr_destination_mv'] ? $model_array['ar_qr_destination_mv'] : $model_array['ar_qr_destination'];
        ...
        if ($ar_qr_destination == 'model-viewer'){
        ...
        } elseif(($ar_qr_destination == 'parent-page')AND(isset($model_array['ar_model_atts']))){
            ...
            $ar_qr_url = get_permalink( get_the_ID() );
        ...
        if(trim($model_array['ar_qr_destination_url'])){
            $ar_qr_url = $model_array['ar_qr_destination_url'];
        }
    ...
    public function get_ar_popup_html($model_array){
        ...
        if (strpos($ar_model_id, ',') !== false) {
            ...
            $ar_atts_id_array = explode(',', $ar_atts_id);
            $ar_atts_id = str_replace(" ","",implode("_",$ar_atts_id_array));
        }

        

        if ($model_array['ar_hide_reset']!=1){
        $output.='<div class="ar-reset-btn-container">
            <button id="ar-reset_'.$ar_model_id.'" class="ar-reset" onclick="document.getElementById(\'model_'.$ar_model_id.'\').setAttribute(\'camera-orbit\', \''.$model_array['ar_camera_orbit_reset'].'\');return getData()"><img src="'.esc_url( plugins_url( "assets/images/reset.png", dirname(__FILE__) ) ).'"></button>
            </div>';
        }
        ...
        if ((!isset($model_array['ar_hide_fullscreen']))OR($model_array['ar_hide_fullscreen']=='')){
            if ($model_array['ar_pop']=='pop'){
                ...
                $output.='<button id="ar_close_'.$ar_atts_id.'" class="ar_popup-btn '.$ar_show_close_on_devices.'" onclick="document.getElementById(\'ar_popup_'.$ar_atts_id.'\').style.display = \'none\';"><img src="'.esc_url( plugins_url( "assets/images/close.png", dirname(__FILE__) ) ).'" class="ar-fullscreen_btn-img"></button>';
            }else{
                ...
                $output.='<button id="ar_pop_Btn_'.$ar_model_id.'" class="ar_popup-btn hide_on_devices" type="button"><img src="'.$ar_fullscreen_image.'" class="ar-fullscreen_btn-img"></button>';
            }
        }
        $output.='</div>';
    ...
    public function get_ar_slider_html($model_array){
        ...
        if ($model_array['ar_model_list']!=''){ 
            ...
            if (count($model_array['ar_model_list'])>1){ 
                ...
                foreach ($model_array['ar_model_list'] as $k =>$v){
                    ...
                    if (get_post_meta( $v, '_ar_poster'.$suffix, true )){
                        $ar_poster=get_post_meta( $v, '_ar_poster'.$suffix, true ).'"';
                    }else{
                        $ar_poster = get_the_post_thumbnail_url($v);
                    }
                    if ($glb_file != '' ){
                        $output.='<button id="ar_btn_'.$v.'" class="ar_slide '.$slide_selected.'" onclick="switchSrc(\'model_'.$ar_model_id.'\', this, \''.$glb_file.'\', \''.ar_get_secure_model_url(get_post_meta($v, '_usdz_file'.$suffix, true )).'\')" style="background-image: url(\''.esc_url( $ar_poster ).'\');"></button>
                        ';
                    }
                }
                $close_div='</div>
        ...
        $output.='<input type="hidden" id="src_'.$ar_model_id.'" value="'. ar_get_secure_model_url($model_array['glb_file']).'">';
    ...
    public function get_ar_hotspots($model_array){
        ...
        if ($model_array['ar_hide_dimensions']==''){
            $ar_dimensions_display = '';
        }else{
            $ar_dimensions_display = ' nodisplay';
        }
        //$output .= '<style>div#controls.dimension{display: '.$ar_dimensions_display.' !important;}</style>';
        $ar_dimensions_label = esc_html(__('Dimensions', 'ar-for-woocommerce' ));
        ...
        <button slot="hotspot-dim-X-Y" class="dimension nodisplay" data-position="-1 -1 0" data-normal="-1 0 0"></button>
        <button slot="hotspot-dot-X-Y+Z" class="dot nodisplay" data-position="-1 -1 1" data-normal="-1 0 0"></button>
        <div id="controls" class="dimension ar-dimension-control'.$ar_dimensions_display.'">
            <label for="show-dimensions_'.$ar_model_id.'" style="margin:0px !important;">'.$ar_dimensions_label.':</label>
            <input id="show-dimensions_'.$ar_model_id.'" type="checkbox" style="cursor: pointer;">
        </div>';
    ...
    public function get_ar_custom_css($model_array){
        ...
        $output = '';
        //ob_start();
        // Define the file path for the dynamic CSS
        $css_handle = 'ar-custom-style';
        $css_file_name = 'uploads/ar-custom-styles.css';
        $css_file = WP_CONTENT_DIR . '/' . $css_file_name;
        //Check if overides is set OR file doesn't exit
        //if (($model_array['ar_css_override'] != '') OR(!file_exists($css_file))) {
        if ($model_array['ar_css_override'] != ''){
            $css_handle .= '_'.$model_array['id'];
            $css_file_name = 'uploads/ar-custom-styles_'.$model_array['id'].'.css';
        }
        // Check if the custom CSS positions are set
        if ($model_array['ar_css_positions'] != '') {
            if (is_array($model_array['ar_css_positions'])) {
                ...
                foreach($model_array['ar_css_positions'] as $element => $pos) {
                    //echo $element.'<br />';
                    if (($pos != 'Default') && ($element != '') && ($pos != '')) {
                        if($element == 'Dimensions'){
                            $output .= esc_attr($ar_css_names[$element]) . '{' . esc_attr($ar_css_styles[$pos]) . ' transform: translateX(0%) !important; }';
                        } else {
                            $output .= esc_attr($ar_css_names[$element]) . '{' . esc_attr($ar_css_styles[$pos]) . '}';
                        }
                    }
                }
    
                // More custom CSS
                if ((isset($model_array['ar_show_close_on_devices'])) && ($ar_no_move_ar_button != '1')) {
                    $output .= '#ar-button_' . esc_html($model_array['model_id']) . '_pop{top:40px !important;}';
                }
    
                // Further conditions for CSS output
                /*if (($ar_no_move_ar_reset != '1') && ($ar_no_move_ar_dimensions != '1') && ($model_array['ar_hide_reset'] != '1')) {
                    $output .= '.dimension{left:50px !important;}';
                }*/
            }
        }
        // Check if the custom Progress Bar color is set
        $ar_progress_bar_color = get_option('ar_progress_bar_color');
        if ($ar_progress_bar_color != '') {
            $output.= '/* Custom loading bar style for model-viewer */ model-viewer::part(default-progress-bar) { background-color: '.$ar_progress_bar_color.'; /* custom color */} /* Customize the percentage progress bar */ model-viewer::part(default-progress-bar)::-webkit-progress-bar { background-color: #f0f0f0; /* Light gray background */ } model-viewer::part(default-progress-bar)::-webkit-progress-value { background-color: '.$ar_progress_bar_color.'; /* Custom color for progress */ } ';
            
        }
        // Add any additional CSS
        if (($model_array['ar_css'] != '') && ($model_array['ar_pop'] != 'pop')) {
            $output .= esc_attr($model_array['ar_css']);
        }
        ...
        // Save the generated CSS to a file
            ...
            // Now use WP_Filesystem to write to the file
            if ( ! empty( $wp_filesystem ) ) {
                // Define the file path
                $file_path = WP_CONTENT_DIR . '/' . $css_file_name;                
                // Use WP_Filesystem to write the contents to the file
                $wp_filesystem->put_contents( $file_path, $output );
            } 
        }
        
        if (file_exists(WP_CONTENT_DIR. '/' . $css_file_name)){
            // Enqueue the dynamic CSS file
            $lastmodtime= filemtime(WP_CONTENT_DIR . '/' . $css_file_name);           
            wp_enqueue_style($css_handle, content_url($css_file_name), array(), $lastmodtime);
        }else{
            // Enqueue the custom CSS file within the plugin
            wp_enqueue_style('ar_styles_custom', plugins_url('ar-for-woocommerce/assets/css/ar-display-custom.css'), false, '1.0.0', 'all'); 
        }
        
    
        return '';  // Return empty as we're enqueueing the styles now
    }
    ...
    public function ar_alternative_model($alt_id, $suffix=''){        
        ...
        if (get_post_meta( $atts['id'], '_ar_poster'.$suffix, true )){
            $model_array['ar_poster']=get_post_meta( $atts['id'], '_ar_poster'.$suffix, true ).'"';
        }else{
            $model_array['ar_poster']='';
        }
    ...
    public function sanitize_atts($atts){
        ...
        foreach($atts as $index => $value){
            if($index == 'id'){
                $temp[$index] = preg_replace('/[^0-9,]/', '', $value);
            } else {
                $temp[$index] = esc_attr(strip_tags(esc_html($atts['text'])));
            }
        }
