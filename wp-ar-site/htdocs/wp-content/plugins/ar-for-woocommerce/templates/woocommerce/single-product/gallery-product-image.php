...
$columns           = esc_attr(apply_filters( 'woocommerce_product_thumbnails_columns', 4 ));
$post_thumbnail_id = $product->get_image_id();
$wrapper_classes   = esc_attr(apply_filters(
...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>" data-columns="<?php echo esc_attr( $columns ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;">
	<div class="woocommerce-product-gallery__wrapper">
		<?php
		// WAPD AR Model Gallery
        if ( $post_thumbnail_id ) {
			//$html = wc_get_gallery_image_html( $post_thumbnail_id, true );
			$html = '<div class="woocommerce-product-gallery__image--placeholder">';
            $html .= do_shortcode('[ar-gallery]');
            $html .= '</div>';
            
		}elseif (get_post_meta($product->get_id(), '_glb_file', true) !== null) {
            $html = '<div class="woocommerce-product-gallery__image--placeholder aaa1">';
