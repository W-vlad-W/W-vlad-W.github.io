...
function ar_for_woocommerce_gutenberg_block_block_init() {
	 register_block_type( __DIR__ . '/build/modelblock', array(
        'render_callback' => 'arwc_gutenberg_block_callback',
        'attributes'      => array(
            'id' => array(
                'type'    => 'number',
                'default' => 0,
...
add_action( 'init', 'ar_for_woocommerce_gutenberg_block_block_init' );


function arwc_block_categories( $categories ) {
    ...
    return in_array( 'ar_display', $category_slugs, true ) ? $categories : array_merge(
        ...
        array(
            array(
                'slug'  => 'ar_display',
                'title' => __( 'AR Display', 'ar-for-woocommerce' ),
                'icon'  => null,
            ),
        )
    );
}
add_filter( 'block_categories_all', 'arwc_block_categories' );



function arwc_gutenberg_block_callback($attributes){
    ob_start();
...
function arwc_gallery_block_callback($attributes){
    global $post;
    ob_start();
...
function arwc_upload_block_callback($attributes){
    global $post;
    ob_start();
